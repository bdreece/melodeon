...
export const client = derived<typeof token, SpotifyApi>(token, ($token, set) => {
    if ($token) {
        const sdk = SpotifyApi.withAccessToken(import.meta.env.VITE_SPOTIFY_CLIENT_ID, $token);
        set(sdk);
    }
});
...
export const device = derived<typeof player, string | undefined>(player, ($player, set) => {
...
>(player, ($player, set) => {
    $player?.addListener('player_state_changed', playerState => {
        $player.getCurrentState().then(playbackState => set({
            ...playerState,
            ...playbackState,
        }));
    });
...
derived<[
    ...
    client: SpotifyApi,
    device: string
}>([client, device], ([$client, $device], set) => set({
    client: $client,
    device: $device,
