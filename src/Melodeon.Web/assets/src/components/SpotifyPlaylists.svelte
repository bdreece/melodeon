...
<script lang="ts">
    ...
    function playPlaylist(uri: string) {
        client?.player
            .getPlaybackState()
            .then(state =>
                client.player.startResumePlayback(state.device.id!, uri),
            )
            .catch(e => console.error(e));
    }
</script>
...
<div class="mb-4 h-96 overflow-scroll">
    <ul
        class="menu grid sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-6 bg-base-200 rounded-box m-2"
    >
        {#each playlists as playlist}
            <li>
                <button
                    type="button"
                    on:click={() => playPlaylist(playlist.uri)}
                >
